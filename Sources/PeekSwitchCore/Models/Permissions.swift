...
/// The three authorizations PeekSwitch needs, and nothing else
...
enum Authorization: String, CaseIterable, Identifiable, Sendable {
    ...
    var purpose: String {
        switch self {
        case .inputMonitoring:
            return "Lets PeekSwitch notice when you hold your extra mouse button. Without it, only the keyboard shortcut opens the switcher."
        case .accessibility:
            return "Lets PeekSwitch list individual windows and raise the exact one you pick. Without it, PeekSwitch can only bring a whole app forward."
        case .screenRecording:
            return "Lets PeekSwitch show a thumbnail of each window. Without it, cards show app icons instead of previews."
        }
    }
