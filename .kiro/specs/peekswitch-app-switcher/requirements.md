...
- **PeekSwitch_App**: The complete macOS application bundle, including all components below.
...
6. THE Window_Registry SHALL restrict Window_Entries to windows owned by applications other than the PeekSwitch_App.
...
6. WHERE a Window_Entry carries no last-activation timestamp from the current PeekSwitch_App session, THE MRU_Tracker SHALL assign an initial timestamp derived from the window's front-to-back position in the system window list, with frontmost windows receiving the most recent timestamps.
...
1. WHEN the PeekSwitch_App launches, THE Trigger_Monitor SHALL install a CGEventTap that observes Other_Mouse_Events system-wide.
...
**User Story:** As a user whose mouse buttons are claimed by another tool, I want a keyboard shortcut that opens the same switcher, so that PeekSwitch stays usable while I sort out my mouse configuration.

#### Acceptance Criteria

1. WHEN the PeekSwitch_App launches, THE Trigger_Monitor SHALL register the Global_Hotkey as a secondary trigger.
...
8. THE Settings_Window SHALL report whether the configured Global_Hotkey has been observed to fire at least once since registration, because successful registration does not establish that the combination is reaching the PeekSwitch_App.
...
**User Story:** As a first-time user, I want PeekSwitch to tell me exactly which permissions it needs and why, so that I can get it working without hunting through System Settings.

#### Acceptance Criteria

1. WHEN the PeekSwitch_App launches, THE Permissions_Manager SHALL determine the current authorization status of each of the Required_Authorizations.
...
6. WHILE all of the Required_Authorizations are granted, THE Permissions_Manager SHALL start the PeekSwitch_App without presenting the Onboarding_Window.
...
**User Story:** As a user of a background utility, I want PeekSwitch to live in the menu bar and stay out of my Dock and app switcher, so that it behaves like the small tool it is.

#### Acceptance Criteria

1. THE PeekSwitch_App SHALL declare itself a user interface element agent so that macOS omits the PeekSwitch_App from the Dock and from the system application switcher.
2. WHEN the PeekSwitch_App launches, THE Menu_Bar_Controller SHALL display a status item in the system menu bar.
...
5. WHEN the user selects the quit control, THE PeekSwitch_App SHALL terminate within 1 second.

### Requirement 12: Version 1 Settings

**User Story:** As a user with a specific mouse and specific habits, I want to choose which button triggers the switcher, how it behaves when I press it, and how many windows it lists, so that PeekSwitch fits my hardware rather than the other way round.
...
8. WHEN the PeekSwitch_App launches with no persisted settings, THE Settings_Store SHALL apply a Trigger_Button value of middle mouse button, a History_Depth value of 10, an Activation_Mode of Automatic, and the default Global_Hotkey.
...
10. WHEN the user changes the Activation_Mode or the Global_Hotkey, THE PeekSwitch_App SHALL apply the change without an application restart.
...
3. WHILE the Overlay is hidden, THE PeekSwitch_App SHALL consume less than 1 percent of one CPU core measured as a 60-second average on the Reference_Machine.
4. WHILE the Overlay is hidden, THE PeekSwitch_App SHALL occupy less than 80 MB of resident memory on the Reference_Machine.
...
1. THE PeekSwitch_App SHALL perform every function using local system APIs, with zero outbound network connections.
2. THE PeekSwitch_App SHALL ship with zero analytics, telemetry, or crash-reporting components.
...
5. THE PeekSwitch_App SHALL request only the Required_Authorizations.
6. THE PeekSwitch_App source repository SHALL include the MIT license text in a LICENSE file at the repository root.
...
1. THE PeekSwitch_App SHALL declare a minimum deployment target of macOS 15.0.
2. THE PeekSwitch_App SHALL build as a universal binary containing arm64 and x86_64 slices.
3. THE PeekSwitch_App SHALL call macOS 15.0 APIs directly, with zero availability branching for macOS versions earlier than 15.0.
4. THE PeekSwitch_App SHALL be implemented in Swift, using SwiftUI for the Overlay presentation and AppKit for window management and event handling.
