...
final class MenuBarController {
    ...
    private func configureButton() {
        applyAppearance(hasWarning: false, tooltip: "PeekSwitch")
    ...
    private func updateButton(warnings: Set<PermissionsManager.Warning>) {
        ...
        applyAppearance(
            ...
            tooltip: hasWarning ? Self.warningSummary(warnings) : "PeekSwitch"
        )
    ...
    private func applyAppearance(hasWarning: Bool, tooltip: String) {
        ...
        button.setAccessibilityLabel(hasWarning ? "PeekSwitch, needs attention" : "PeekSwitch")
        ...
        if let image = Self.icon(hasWarning: hasWarning) {
            ...
            button.title = hasWarning ? "PeekSwitch !" : "PeekSwitch"
        }
    }
    ...
    private static func icon(hasWarning: Bool) -> NSImage? {
        ...
        let description = hasWarning ? "PeekSwitch needs attention" : "PeekSwitch"
    ...
    private func buildMenu() -> NSMenu {
        ...
        let version = NSMenuItem(title: "PeekSwitch \(Self.versionString)", action: nil, keyEquivalent: "")
        version.isEnabled = false
        menu.addItem(version)

        let quit = NSMenuItem(title: "Quit PeekSwitch", action: #selector(quit), keyEquivalent: "q")
        quit.target = self
        menu.addItem(quit)
