...
enum BrowserTabAlertService {
    ...
    static func readings(forBrowserProcesses processes: Set<pid_t>) -> [TabMediaAlert.Reading] {
        guard !processes.isEmpty, AXIsProcessTrusted() else { return [] }

        var readings: [TabMediaAlert.Reading] = []
        for pid in processes {
            let application = AXUIElementCreateApplication(pid)
            AXBridge.applyMessagingTimeout(application)

            for strip in tabStrips(under: application) {
    ...
    private static func tabStrips(under application: AXUIElement) -> [AXUIElement] {
        var frontier = AXBridge.elements(application, kAXWindowsAttribute as String) ?? []
        var found: [AXUIElement] = []
        var depth = 0

        while !frontier.isEmpty, depth < maximumStripDepth, found.isEmpty {
            ...
            for element in frontier {
                ...
                guard role == "AXWindow" || role == "AXGroup" || role == "AXToolbar" else { continue }
                next.append(
