...
/// Command-line self-check: `PeekSwitch --probe`.
///
...
public enum Diagnostics {
    ...
    public static func runProbe() {
        print("PeekSwitch \(MenuBarController.versionString) diagnostics")
    ...
    private static func reportPermissions() {
        ...
        if !permissions.allGranted {
            print("""
                  ...
                  PeekSwitch — Screen Recording especially. A "not granted" line here
                  may be your terminal's status, not the app's.

                  For the app's own reading:
                    log show --predicate 'subsystem == "dev.peekswitch.PeekSwitch"' \\
    ...
    private static func reportTapLocation() {
        ...
        if monitor.install() {
            ...
            print("  (when run from a terminal this may reflect the terminal, not PeekSwitch)")
        }
    }
